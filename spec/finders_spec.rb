require 'spec_helper'
require 'leaf/finders/base'

class Model
  extend Leaf::Finders::Base
end

describe Leaf::Finders::Base do
  ...
  it "should result with Leaf::Collection" do
    Model.expects(:wp_query)
    Model.paginate(:page => nil).should be_instance_of(Leaf::Collection)
  end

  it "should delegate pagination to wp_query" do
    Model.expects(:wp_query).with({}, instance_of(Leaf::Collection), [])
